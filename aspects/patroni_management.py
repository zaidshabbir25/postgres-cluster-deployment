...
from aspects import auth_setup, pg_server_management, service_management
...
def build_config(plan, node):
    ...
    config = {
        ...
        "etcd3": {"hosts": list(plan.etcd_endpoints)},
    }
...
def start(executor, plan, node, run_logger=None):
    ...
    executor.try_run(
        f"nohup {executor.sudo_prefix()} -u {plan.db_user} bash -c "
