...
Topology
  ...
  --sync-mode async|sync|quorum
                            how a node replicates to its standby: async can
                            lose recent commits on failover, sync waits for
                            the standby to confirm them                [async]
  --sync-count N            synchronous standbys per node                  [1]
  --sync-strict             refuse writes while no standby is available,
                            instead of falling back to async
...
Growing a running cluster
  ...
  --sync-mode async|sync|quorum
                            with --role standby, how its leader replicates to
                            it                                 [the cluster's]
  --sync-count N            with --role standby, synchronous standbys in that
                            scope                                          [1]
  --sync-strict             with --role standby and a synchronous mode, refuse
                            writes while no standby is available
...
ask_sync_mode() {
  # Sets SYNC_MODE / SYNC_COUNT / SYNC_STRICT from the answers. Only worth
  # asking where a standby exists: a scope with none replicates to nobody.
  #   $1 = how many standbys the answer applies to (for the count question)
  local standby_count="${1:-1}"
  say "   ${DIM}A standby can be asynchronous or synchronous. Asynchronous never"
  say "   makes a commit wait, but a failover can lose the last few. Synchronous"
  say "   holds each commit until the standby confirms it — no loss, at the cost"
  say "   of a round trip per write.${RESET}"
  say ""
  if [[ "$(ask_choice "   Synchronous replication to the standby? (y/n)" "n" y n)" != "y" ]]; then
    SYNC_MODE="async"
    say ""
    return 0
  fi
  ...
  if (( standby_count > 1 )); then
    SYNC_COUNT="$(ask_int "   How many standbys must confirm each commit" "1" 1 "$standby_count")"
  else
    SYNC_COUNT=1
  fi
  say ""
  say "   ${DIM}If the standby is down, Patroni normally falls back to asynchronous"
  say "   so writes keep flowing. Strict mode refuses writes instead, which"
  say "   guarantees durability and costs availability.${RESET}"
  if [[ "$(ask_choice "   Refuse writes when no standby is available? (y/n)" "n" y n)" == "y" ]]; then
    SYNC_STRICT=true
  fi
  say ""
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --sync-mode|--sync-count)
      [[ $# -ge 2 ]] || die "$1 needs a value"
      [[ "$1" == "--sync-mode" ]]  && SYNC_MODE="$2"
      [[ "$1" == "--sync-count" ]] && SYNC_COUNT="$2"
      shift 2 ;;
...
if [[ -n "$ADD_NODE" ]]; then
  ...
  # --- how the leader replicates to it --------------------------------
  # Whether the leader came from a flag or the question above, a standby is
  # the only thing this setting describes.
  if [[ "$NODE_ROLE" == "standby" ]]; then
    if [[ -z "$SYNC_MODE" ]]; then
      say "${BOLD}3) How should $NODE_LEADER replicate to $ADD_NODE?${RESET}"
      ask_sync_mode 1
    fi
    NODE_ARGS+=(--sync-mode "$SYNC_MODE")
    [[ -n "$SYNC_COUNT" ]] && NODE_ARGS+=(--sync-count "$SYNC_COUNT")
    [[ "$SYNC_STRICT" == true ]] && NODE_ARGS+=(--sync-strict)
  elif [[ -n "$SYNC_MODE" || -n "$SYNC_COUNT" || "$SYNC_STRICT" == true ]]; then
    die "--sync-mode, --sync-count and --sync-strict describe how a leader replicates to its standbys; they apply with --role standby"
  fi
  ...
  if [[ "$NODE_ROLE" != "standby" ]]; then
    if [[ -z "$NODE_SPOCK_MAJOR" ]]; then
      ...
      say "   ${DIM}The cluster runs spock${FACT_SPOCK_MAJOR}. A newer Spock on the new node is"
      say "   supported — it can read an older peer's stream, not the reverse —"
      say "   so this may match the cluster or be newer, never older.${RESET}"
      say ""
      NODE_SPOCK_MAJOR="$(ask_choice "   Spock major version (50 or 60)" "$FACT_SPOCK_MAJOR" 50 60)"
      say ""
    fi
    if (( NODE_SPOCK_MAJOR < FACT_SPOCK_MAJOR )); then
      die "the cluster runs spock$FACT_SPOCK_MAJOR; a new node may not run an older Spock than its peers. Use spock$FACT_SPOCK_MAJOR or later"
    fi
    if [[ "$NODE_SPOCK_MAJOR" != "$FACT_SPOCK_MAJOR" ]]; then
      say "   ${DIM}Mixed-version add: the cluster moves from spock$FACT_SPOCK_MAJOR to spock$NODE_SPOCK_MAJOR one node"
      say "   at a time. Every existing node needs Spock 5.0.9 or later.${RESET}"
      say ""
    fi
...
if [[ "$INTERACTIVE" == true ]]; then
  ...
  if [[ -n "$STANDBY" ]]; then
    say ""
    STANDBY_COUNT=$(printf '%s' "$STANDBY" | tr ',' '\n' | grep -c .)
    ask_sync_mode "$STANDBY_COUNT"
  fi
  say ""
  ...
  [[ -n "$SYNC_COUNT" ]] && PLAN_ARGS+=(--sync-count "$SYNC_COUNT")
  [[ "$SYNC_STRICT" == true ]] && PLAN_ARGS+=(--sync-strict)
